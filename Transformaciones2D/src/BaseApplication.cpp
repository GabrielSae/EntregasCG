...
void BaseApplication::putPixel_O(const int& x, const int& y, const char& R, const char& G, const char& B, const char& A)
{
...
{
	putPixel_O(x + WIDTH / 2, y + HEIGHT / 2, R, G, B, A);
}

void BaseApplication::putPixel1(const int& x, const int& y, Color c)
{
	putPixel(x - WIDTH / 2, y - HEIGHT / 2, c.m[0], c.m[1], c.m[2], c.m[3]);
}
...
void BaseApplication::drawLine(const float& x0, const float& y0, const float& x1, const float& y1) {
	int dx = (int)(x1 - x0);
	int dy = (int)(y1 - y0);
	...
	putPixel((int)x0, (int)y0, 255, 255, 255, 0);
	//putPixel((int)x0, (int)y0);
